...
void hw_global_set_input_fn(char *input_fn) {
  hw_global->input_fn = (char *)malloc(std::strlen(input_fn) + 1);
  std::memcpy(hw_global->input_fn, input_fn, std::strlen(input_fn) + 1);
  char *pfnc = input_fn;
  ...
  hw_global->output_fn =
      (char *)calloc(std::strlen(pfnc) + strlen(cwd) + 6 + 1, sizeof(char));
  std::strcat(hw_global->output_fn, cwd);
  std::strcat(hw_global->output_fn, "/stat/");
  std::strcat(hw_global->output_fn, pfnc);
}
